...
class Apkg:
    ...
    def extract(self, package, dest_dir=None):
        ...
        if apkg_version == '1.0':
            ...
            print 'Unsupported apkg version'
            shutil.rmtree(tmp_dir, ignore_errors=True)
            return -1

        if os.path.isdir(app_dir):
            print 'The folder exists, please remove it: %s' % app_dir
            shutil.rmtree(tmp_dir, ignore_errors=True)
            return -1
        else:
    ...
    def convert(self, package):
        ...
        control_dir = app_dir + '/' + self.apk_special_folders['control']
        config_file = control_dir + '/' + self.apk_control_files['pkg-config']
        changelog_file = control_dir + '/' + self.apk_control_files['changelog']
    ...
    def upload(package):
        ...
        print 'function not supported: %s' % ('upload')
